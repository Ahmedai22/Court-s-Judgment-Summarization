...
#     count_line, OUTPUT_DIR, safe_division, get_word2rank, remove_stopwords, remove_punctuation
# import Levenshtein
# import argparse
...
#     ), 2))
# ### Speicial tokens end
# last_layer_norm_output = None
# def hook_fn(module, input, output):
#     global last_layer_norm_output
#     last_layer_norm_output = output
...
#         # print(self.args.sum_model, self.args.sim_model)
...
#         self.summarizer.model.decoder.layernorm_embedding.register_forward_hook(hook_fn)
...
#             {
#                 "params": [p for n,p in self.summarizer.named_parameters()]
#             },
#             {
#                 "params": [p for n,p in self.simplifier.named_parameters()]
#             },
#             {
...
#         source = batch["source"]
#         labels = batch['target_ids'].to(self.args.device)
#         decoder_mask = batch['target_mask'].to(self.args.device)
        
#         # print(source)
#         # exit()
        
#         with torch.no_grad():
#             tgt_sum_outputs = self.summarizer(
#                 input_ids = labels,
...
#         # print(labels.device, batch['target_mask'].device)
...
#         sim_outputs  = self.simplifier(
#             input_ids = padded_summary_ids,
#             attention_mask = summary_attention_mask,
#             # inputs_embeds = last_layer_norm_output,
#             labels = labels,
#             decoder_attention_mask = decoder_mask
#         )
        
#         H2 = tgt_sum_outputs.encoder_last_hidden_state
        
#         ## CosSim
#         # Rep1 = torch.matmul(H1.permute(0,2,1), self.W).permute(0,2,1)
#         Rep1 = self.W(H1.permute(0,2,1)).permute(0,2,1)
...
#         loss, sari = self.sari_validation_step(batch)
#         # loss = self._step(batch)
#         # print("Val_loss", loss)
#         logs = {"val_loss": loss}

#         self.log('val_loss', loss, batch_size = self.args.valid_batch_size)
#         return torch.tensor(loss, dtype=float), sari
...
#     def save_core_model(self, epoch):
#       self.args.model_name = 'simsum_bart_'
      
#       tmp = self.args.model_name + str(epoch)+'.pt'
#       store_path = self.args.output_dir / tmp
#       torch.save(self.state_dict(), store_path)
#     #   self.save_pretrained(store_path)
...
#       p.add_argument('-device','--device', default = 0, type=int)
...
#         print(f"Training Loss: {total_train_loss/idx}")
        
#         val_loss = 0
#         val_sari = 0
#         model.eval()
#         for idx,batch in enumerate(tqdm(val_dataloader)):
#             loss, sari = model.validation_step(batch)
#             val_loss += loss
#             val_sari += sari
        
#         print(f"SARI score: {val_sari/idx}")
        
#         if(prev_sari < val_sari):
#             print("Saving model")
#             model.save_core_model(epoch)
#             prev_sari = val_sari
...
class SumSim(pl.LightningModule):
    def __init__(self,args):
        ...
        #WIKI_DOC
        # self.simplifier = BartFineTuner.load_from_checkpoint("./experiments/exp_1711724803164617/checkpoint-epoch=4.ckpt")
        
        #D_WIKI
        self.simplifier = BartFineTuner.load_from_checkpoint("./experiments/exp_1711865355702580/checkpoint-epoch=5.ckpt")
