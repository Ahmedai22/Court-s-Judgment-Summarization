...
# next two functions written for mildsum train dataset
def read_lines(filepath):
...
# next two functions are for mildsum test dataset or evaluate.py 
...
# Writing next function for mildsum dataset:
def get_data_filepath(dataset, phase):
    path = os.path.join(DATASETS_DIR, dataset, phase)
    return path
