...
# SIMSUM (Bart_finetuned)
# model_dirname = 'exp_1711784152965817'
# checkpoint_path = 'checkpoint-epoch=4.ckpt'

# SIMSUM (T5_finetuned)
# model_dirname = 'exp_1711865523788124'
# checkpoint_path = 'checkpoint-epoch=4.ckpt'
...
def evaluate_on_MILDSUM(phase, features_kwargs=None,  model_dirname = None):
    ...
    complex_filepath =get_data_filepath(dataset, 'test',)# _kw_num3_div0.9'
    
    if not output_score_filepath.exists() or count_line(output_score_filepath)==0:
        start_time = time.time()
        complex_filepath =get_data_filepath(dataset, 'test')
...
p = ArgumentParser()
p.add_argument('--dataset', default='WIKI_DOC', type=str, help="select dataset")
p.add_argument('--phase', default='test', type=str, help="select dataset phase") # complex / simple

args,_ = p.parse_known_args()


if args.dataset == 'WIKI_DOC':
        evaluate_on_WIKIDOC(phase=args.phase, features_kwargs=None, model_dirname=model_dirname)
else:
        evaluate_on_D_WIKI(phase=args.phase, features_kwargs=None, model_dirname=model_dirname)
